...
void initCells(Cell *board[][10], int boardSize)
{
    for (int i = 0; i < boardSize; ++i)
    {
        ...
        { // nested for loop to access arrays and initalize the cells
            board[i][j] = new Cell;
            board[i][j]->x = i;
            board[i][j]->y = j;
...
{
    // fileio read in, reads in array through nested for loop
    string chosenFile;
...
bool updateCellState(Cell *board[][10], int boardSize)
{
    ...
    if (!updated) // if updated is false, print no more stages
    {
